...
// LeetCode-derived flag only — duel links carry no ?country= override.
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if ("card" in a && "card" in b) {
    ...
    return {
      ...
      description: `Six stats, one result: @${a.card.login} vs @${b.card.login}, settled on real LeetCode numbers.`,
...
function MatchPostponed({
  ...
  return (
    <main className="relative z-[2] mx-auto flex min-h-screen max-w-[560px] flex-col items-center justify-center px-6 text-center">
      ...
      <Link
        href="/"
        className="font-display mt-7 inline-flex h-[46px] items-center rounded-xl bg-brand px-6 text-[16px] tracking-[.06em] text-[#1a1305] transition hover:bg-brand-hi"
      >
        BACK TO THE BENCH
      </Link>
    </main>
...
export default async function Page({ params }: Params) {
  ...
  after(() => Promise.all([recordScout(username), recordScout(opponent)])); // a duel is two scouts
