...
// NEXT_PUBLIC_SITE_URL; the leetfut.com fallback keeps forks/dev working unset.
// NEXT_PUBLIC_* is inlined at build time, so this works in client modules
// (lib/share.ts) as well as on the server.
export const SITE_URL = (process.env.NEXT_PUBLIC_SITE_URL || "https://leetfut.com").replace(/\/+$/, "");

// Bare host for display copy (OG-image footers): "leetfut.com".
