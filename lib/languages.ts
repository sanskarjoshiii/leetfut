// Top-language → logo resolution. Pure, framework-agnostic (no DOM): given a
// list of {language} entries (one per problem/submission-language bucket), rank
// languages by how many entries use each, then resolve a colored logo from
// Devicon (https://devicon.dev), served by jsDelivr — ~150 languages (so Rust,
// Go, C++, Kotlin, Dart… all resolve).
//
// We still DEMOTE styling/markup/data/prose languages so a card's headline goes
// to a real programming language (see NON_HEADLINE).
//
