...
// of a uniform random sample of LeetCode users (scored by this same engine),
...
export default function DistributionPanel({ card }: { card: Card }) {
  ...
  const tipText =
    `Higher than ${(DIST_N - all.atOrAbove).toLocaleString()} of ${DIST_N.toLocaleString()} randomly sampled LeetCode users, ` +
  ...
  return (
    <section className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-[16px]">
      ...
      <svg viewBox={`0 0 ${W} ${H + 16}`} className="w-full" role="img" aria-label={`Overall rating ${card.overall} versus a random sample of ${DIST_N} LeetCode users`}>
        {DIST_COUNTS.map((c, i) =>
          c === 0 ? null : (
            <rect
              key={i}
