...
export default function AppShell({
  ...
  scoutHistory: ScoutHistoryPoint[] | null;
}) {
  ...
  return (
    <>
      {/* `isolate` gives the whole content layer its own compositing group, so a
          hovered card's blend-mode glass + transforms never force the background
          video (behind this layer) to re-composite and hitch/restart. */}
      <main className="relative isolate z-[2] flex min-h-screen flex-col">
        ...
        {/* page nav, overlaid top-left to mirror the GitHub star — never pushes
            the vertically-centered hero down. */}
        <div className="absolute left-[clamp(20px,5vw,52px)] top-[clamp(16px,3vh,26px)] z-[3] max-w-[calc(100%-140px)]">
          <FooterLinks />
        </div>
