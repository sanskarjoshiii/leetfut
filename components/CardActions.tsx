...
export default function CardActions({
  ...
  const { canNativeShare, nativeShare, copyLink } = useShareActions({
    getSharePayload: async () => {
      ...
      if (node && "canShare" in navigator) {
        ...
        if (blob) {
          const file = new File([blob], `${card.login}-leetfut.png`, { type: "image/png" });
  ...
  const track = async (id: ActionId, run: () => Promise<void>) => {
    ...
    try {
      ...
      console.error(`[leetfut] card ${id} failed:`, e);
  ...
  const downloadPng = () =>
    track("download", async () => {
      ...
      a.download = `${card.login}-leetfut.png`;
      a.href = url;
      a.click();
    });
  ...
  const shareStory = () =>
    track("story", async () => {
      ...
      const file = new File([blob], `${card.login}-leetfut-story.png`, { type: "image/png" });
